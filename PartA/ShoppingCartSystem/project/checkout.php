   ...
   if(isset($_GET['get_id'])){
      try {
         ...
         if($get_product->rowCount() > 0){
            while($fetch_p = $get_product->fetch(PDO::FETCH_ASSOC)){
               ...
               $insert_order->execute([create_unique_id(), $user_id, $name, $number, $email, $address, $address_type, $method, $fetch_p['id'], $fetch_p['price'], 1, date('Y-m-d H:i:s'), 'pending']);
               if($insert_order){
      ...
      try {
         while($f_cart = $verify_cart->fetch(PDO::FETCH_ASSOC)){
            $insert_order = $conn->prepare("INSERT INTO `orders`(id, user_id, name, number, email, address, address_type, method, product_id, price, qty, date, status) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)");
            $insert_order->execute([create_unique_id(), $user_id, $name, $number, $email, $address, $address_type, $method, $f_cart['product_id'], $f_cart['price'], $f_cart['qty'], date('Y-m-d H:i:s'), 'in progress']);
         }
      ...
      <form action="" method="POST">
         ...
         <div class="flex">
            <div class="box">
               ...
               <input type="number" name="number" required maxlength="15" placeholder="enter your number" class="input" min="0" max="999999999999999">
